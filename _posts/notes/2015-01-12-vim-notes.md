...
- Fugitive
    ...
    - `Gdiff HEAD^3:path/to/a/file`: compare the current file with a file in 3 commits before the current HEAD.
...
# Compiling VIM from the source with X

Compiling vim from source with X option was very tricky.

Here is the recipe that worked for me.

{% highlight bash %}
sudo apt-get build-dep vim
./configure --enable-pythoninterp --with-features=huge --with-x --enable-gui=auto
make distclean
make
sudo make install
{% endhighlight %}
